...
int mock_counter_open, mock_counter_close, mock_counter_dup, mock_counter_dup2,
    mock_counter_pipe, mock_counter_fork, mock_counter_execvp,
    mock_counter_exit, mock_counter_wait, mock_counter_waitpid,
    mock_counter_chdir;
...
#define PIPE_NAME_LEN                                                          \
    3 /* Debe valer que MOCK_FD_TABLE_SIZE <= 10 ^ PIPE_NAME_LEN */
...
static void mock_fd_clear(int fd) {
    ...
    free(mock_fd_table[fd].name);
    mock_fd_table[fd].name = NULL;
...
static int mock_fd_lookup(void) {
    ...
    int result = 0;
    while (result < MOCK_FD_TABLE_SIZE &&
           mock_fd_table[result].kind != KIND_CLOSED)
        result++;
...
bool mock_check_fd(int fd, mock_file_kind kind, const char *name) {
    ...
    return mock_fd_table[fd].kind == kind &&
           (/* El tipo coincide Y */
...
int mock_dup(int oldfd) {
    int result = -1;
    mock_counter_dup++;
    if (oldfd < 0 || oldfd >= MOCK_FD_TABLE_SIZE ||
        mock_fd_table[oldfd].kind == KIND_CLOSED) {
        errno = EBADF; /* Número inválido, o ya estaba cerrado */
    } else {
        result = mock_fd_lookup();
        if (result >= 0) {
            memcpy(mock_fd_table + result, mock_fd_table + oldfd,
                   sizeof mock_fd_table[result]);
...
/* Estas variables se usan por el macro EXIT_PROTECTED. No declaradas como
 * static para que el macro (necesariamente en el .h) pueda accederlas
 */
...
static void finish_process(int i) {
    ...
    if (mock_finished_processes_count < MAX_CHILDREN) {
        mock_finished_processes[mock_finished_processes_count] =
            mock_waitable_processes[i];
