 ...
 * llamadas inválidas
 */
START_TEST(test_destroy_null) { scommand_destroy(NULL); }
END_TEST

START_TEST(test_push_back_null) { scommand_push_back(NULL, strdup("123")); }
END_TEST
...
START_TEST(test_pop_front_null) { scommand_pop_front(NULL); }
END_TEST
...
START_TEST(test_is_empty_null) { scommand_is_empty(NULL); }
END_TEST

START_TEST(test_length_null) { scommand_length(NULL); }
END_TEST

START_TEST(test_front_null) { scommand_front(NULL); }
END_TEST
...
START_TEST(test_get_redir_in_null) { scommand_get_redir_in(NULL); }
END_TEST

START_TEST(test_get_redir_out_null) { scommand_get_redir_out(NULL); }
END_TEST

START_TEST(test_to_string_null) { scommand_to_string(NULL); }
END_TEST
...
static void setup(void) { scmd = scommand_new(); }
...
static char **numbers_as_str(int max_number) {
    assert(max_number > 0);
    char **strings = NULL;
...
START_TEST(test_redir) {
    ...
    ck_assert_msg(
        strcmp(scommand_get_redir_in(scmd), scommand_get_redir_out(scmd)) != 0,
        NULL);
    /* ahora si ambos idem */
...
START_TEST(test_to_string) {
    ...
    assert(MAX_LENGTH > 2);
    /* comando "0 1 2 .... N-3 < N-2 > N-1" tiene que tener todos los números y
     * los dos piquitos */
    ...
    ck_assert_msg(strstr(str, strings[MAX_LENGTH - 2]) > strchr(str, '<'),
                  NULL);
    // Redir out
    ck_assert_msg(strstr(str, strings[MAX_LENGTH - 1]) != NULL, NULL);
    ck_assert_msg(strchr(str, '>') != NULL, NULL);
    ck_assert_msg(strstr(str, strings[MAX_LENGTH - 1]) > strchr(str, '>'),
                  NULL);
    free(str);
    free(strings);
}
...
Suite *scommand_suite(void) {
    ...
    tcase_add_test_raise_signal(tc_preconditions, test_push_back_argument_null,
                                SIGABRT);
    tcase_add_test_raise_signal(tc_preconditions, test_pop_front_null, SIGABRT);
    tcase_add_test_raise_signal(tc_preconditions, test_pop_front_empty,
                                SIGABRT);
    tcase_add_test_raise_signal(tc_preconditions, test_set_redir_in_null,
                                SIGABRT);
    tcase_add_test_raise_signal(tc_preconditions, test_set_redir_out_null,
                                SIGABRT);
    ...
    tcase_add_test_raise_signal(tc_preconditions, test_get_redir_in_null,
                                SIGABRT);
    tcase_add_test_raise_signal(tc_preconditions, test_get_redir_out_null,
                                SIGABRT);
